<template>
    <form action="/api/training-properties/types" method="POST" id="newRunType" @submit.prevent="onSubmit"
          @keydown="form.errors.clear($event.target.name)">

        <div class="field">
            <label class="label" for="name">Run Type Name</label>
            <div class="control">
                <input type="text" class="input" name="name" id="name" v-model="form.name">
                <span id="runTypeNameHelp" class="help is-danger" v-if="form.errors.has('name')"
...
<script>
    export default {
        ...
        methods: {
            onSubmit() {
                this.form
                .post('/api/training-properties/types')
