<template>
    <div v-if="editing" class="columns table-row m-0 px-4">
        ...
        <div class="column">
            <form action="/meets/hosts" method="POST" id="editMeetHost" @submit.prevent="update"
...
<script>
    ...
    export default {
        ...
        methods: {
            update() {
                this.form
                    .patch('/meet_properties/hosts/' + this.data.id)
                    .then(data => {
            ...
            destroy() {
                axios.delete('/meet_properties/hosts/' + this.data.id);
