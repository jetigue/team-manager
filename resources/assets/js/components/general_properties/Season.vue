<template>
    <div v-if="editing" class="columns table-row m-0 px-4">
        ...
        <div class="column">
            <form action="/api/general-properties/seasons/id" method="POST" id="editSeason" @submit.prevent="update"
...
<script>
    export default {
        ...
        methods: {
            update() {
                this.form
                    .patch('/api/general-properties/seasons/' + this.data.id)
            ...
            destroy() {
                axios.delete('/api/general-properties/seasons/' + this.data.id);
