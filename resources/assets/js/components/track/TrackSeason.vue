<template>
    <div v-if="editing" class="columns table-row m-0 px-4">
        ...
        <div class="column">
            <form action="/api/track/seasons/id" method="POST" id="editTrackSeason" @submit.prevent="update"
...
<script>
    export default {
        ...
        methods: {
            update() {
                this.form
                    .patch('/api/track/seasons/' + this.data.id)
                    .then(data => {
            ...
            destroy() {
                axios.delete('/api/track/seasons/' + this.data.id);
