<template>
    <div v-if="editing" class="columns table-row m-0 px-4">
        ...
        <div class="column">
            <form action="/api/race-properties/divisions/id" method="POST" id="editRaceDivision" @submit.prevent="update"
...
<script>
    export default {
        ...
        methods: {
            update() {
                this.form
                    .patch('/api/race-properties/divisions/' + this.data.id)
            ...
            destroy() {
                axios.delete('/api/race-properties/divisions/' + this.data.id);
