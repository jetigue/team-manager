<template>
    <div v-if="editing" class="columns table-row m-0 px-4">
        ...
        <div class="column">
            <form action="/api/race-properties/genders" method="POST" id="editGender" @submit.prevent="update"
...
<script>
    export default {
        ...
        methods: {
            ...
            update() {
                this.form
                .patch('/api/race-properties/genders/' + this.data.id)
                    .then(() => {
            ...
            destroy() {
                axios.delete('/api/race-properties/genders/' + this.data.id);
